...
var createTag = function(src, asset, attributes, version) {
  ...
  var mimeType;
  // Force mime type
  if ('mimeType' in attributes && attributes.mimeType) {
    mimeType = attributes.mimeType;
    delete attributes.mimeType;
  } else {
    mimeType = mime.lookup(asset.split('?')[0]);
  }
  // Check mime type
  switch(mimeType) {
...
var processAssets = function(options, results, done) {
  ...
  async.mapLimit(results, 100, function (result, iter) {
...
var walkThroughViews = function(options) {
  var walker = function () {
    var walker   = walk.walk(options.viewsDir)
      , results  = []
      , regexCDN = /CDN\(((\([^)]+\)|[^)])+)\)/ig
      , dustRegexCDN = /@CDN param="((\([^"]+\)|[^"])+)/ig;
    walker.on('file', function(root, stat, next) {
      var validExts = options.extensions || ['.jade', '.ejs', '.dust'];
      ...
      if (_.indexOf(validExts, ext) !== -1) {
        fs.readFile(path.join(root, stat.name), 'utf8', function(err, data) {
          if (err) throwError(err);
          var regex = (ext === '.dust') ? dustRegexCDN : regexCDN
            , match;
...
module.exports = {
  CDN: CDN,
  walkThroughViews: walkThroughViews,
};
